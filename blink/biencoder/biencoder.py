...
class BiEncoderModule(torch.nn.Module):
    def __init__(self, params):
        ...
        self.ctxt_bert_config = ctxt_bert.config

        bert_output_dim = ctxt_bert.embeddings.word_embeddings.weight.size(1)
        self.mention_aggregation_type = params['mention_aggregation_type']
        if params['mention_aggregation_type'] is not None:
            self.classification_heads = nn.ModuleDict({
                'mention_scores': MentionScoresHead(bert_output_dim),
                'get_context_embeds': GetContextEmbedsHead(params['mention_aggregation_type']),
            })
    ...
    def forward(
        ...
        if token_idx_ctxt is not None:
            if self.mention_aggregation_type is None:
                # don't do mention aggregation
                embedding_ctxt = self.context_encoder(
                    token_idx_ctxt, segment_idx_ctxt, mask_ctxt,
                )
            else:
                bert_output, _ = self.context_encoder.bert_model(
                ...
                if gold_mention_idxs is not None:
                    embedding_ctxt = self.classification_heads['get_context_embeds'](bert_output, gold_mention_idxs)
                else:
                    start_pos = start_logits.max(1)
                    end_pos = end_logits.max(1)
                    pred_mention_idxs = torch.stack([start_pos, end_pos])
                    embedding_ctxt = self.classification_heads['get_context_embeds'](bert_output, pred_mention_idxs)
...
class BiEncoderRanker(torch.nn.Module):
    ...
    def forward(self, context_input, cand_input, label_input=None, gold_mention_idxs=None):
        ...
        scores, start_logits, end_logits = self.score_candidate(
            context_input, cand_input, flag, gold_mention_idxs=gold_mention_idxs,
        )

        span_loss = 0
        if start_logits is not None:
            N, M = context_input.size()  # TODO check
        ...
        if label_input is None:
            ...
            # TODO: add parameters?
            loss = loss_fct(scores, label_input) + span_loss
        return loss, scores

    def get_span_loss(
        self, start_positions, end_positions,
        ...
        start_losses = [loss_fct(start_logits, _start_positions) \
                        for _start_positions in torch.unbind(start_positions, dim=1)]
        end_losses = [loss_fct(end_logits, _end_positions) \
                      for _end_positions in torch.unbind(end_positions, dim=1)]
